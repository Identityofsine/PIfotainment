...
class Processor(object):
	...
	def writetempfile(self, imagebinary):
        #returns the randomly generated photo
		S=6
		ran = self.info.songalbum.strip().replace(' ', '').replace('\\', '') if self.info.songalbum is not None else "temp"
		print(f"filename : {ran}.png")
		if imagebinary:
			if os.path.exists(f"/tmp/coverart/") is False:
				os.makedirs("/tmp/coverart/")
			#if os.path.exists(f"/tmp/coverart/{ran}.png") is False:
			with open(f"/tmp/coverart/{ran}.png", "bw+") as f:
				f.write(imagebinary)
				f.close()
	...
	def process_line(self, line):
		...
		if item.type == "ssnc":
			...
			if item.code == "PICT":  # the payload is a picture, either a JPEG or a PNG. Check the first few bytes to see which.			
				...
				self.info.coverartpng = self.writetempfile(item.data)
			...
			if item.code == "mikd":  # -- the kind of item.  So far, only '2' has been seen, an audio
				...
				self.callback(self.info.coverartpng, self.info.songalbum, self.info.itemname, self.info.songartist)
